...
export default {
    ...
    setup(props, { emit }) {
        ...
        const disabledPrev = computed(() => pagesLink.value[0] <= 0);
        const disabledNext = computed(
            () =>
                pagesLink.value[pagesLink.value.length - 1] + 1 >=
                props.totalPages
        );

        const updateIndex = (number) => {
            emit('updated-index', number);
        };
        ...
        const changePage = (isLeft = false) => {
            if (isLeft && !disabledPrev.value) {
                ...
                if (index < itemStart) {
                    ...
                    pagesLink.value.length > props.amount &&
                        pagesLink.value.pop();
                }
